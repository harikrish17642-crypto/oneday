  ...
  return(
    <div style={{fontFamily:"'DM Sans',sans-serif",color:B.text,minHeight:"100vh",WebkitFontSmoothing:"antialiased",position:"relative",
      ...
      <main style={{maxWidth:1080,margin:"0 auto",padding:"24px 24px 100px"}}>
        ...
        {/* Info: when results found but no Naukri/LinkedIn */}
        {!loading&&searched&&jobs.length>0&&!jobs.some(j=>j.source==="naukri"||j.source==="linkedin")&&(
          <div style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:14,padding:"14px 18px",
            borderRadius:B.radiusSm,background:B.oliveDim,border:`1px solid ${B.olive}20`,animation:"fadeUp .2s ease"}}>
            <Sparkles size={15} color={B.olive} style={{marginTop:2,flexShrink:0}}/>
            <div>
              <span style={{fontSize:13,color:B.text,fontWeight:600}}>Indian job results (Naukri, LinkedIn) require direct access.</span>
              <span style={{fontSize:12.5,color:B.textDim,marginLeft:6}}>
                Use the platform buttons above — they open pre-searched results sorted by newest, with clickable apply links.
              </span>
            </div>
          </div>
        )}
        ...
        {!loading&&searched&&filtered.length===0&&(
          <div style={{textAlign:"center",padding:"50px 20px",animation:"fadeUp .3s ease"}}>
            ...
            <p style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,fontWeight:500,color:B.text,marginBottom:8}}>No matching results from global job boards</p>
            <p style={{fontSize:13.5,color:B.textDim,maxWidth:440,margin:"0 auto 20px",lineHeight:1.6}}>
              For Indian roles, Naukri and LinkedIn have the most listings. Click the buttons above — they'll open with your search pre-filled and sorted by newest.
            </p>
            <div style={{display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap"}}>
              {[
                {label:"Search on LinkedIn",url:linkedinURL(role,locations[0]||""),color:B.linkedin},
                {label:"Search on Naukri",url:naukriURL(role,locations[0]||"",experience),color:B.naukri},
                {label:"Search on Indeed",url:indeedURL(role,locations[0]||""),color:B.indeed},
              ].map(x=>(
                <a key={x.label} href={x.url} target="_blank" rel="noreferrer"
                  style={{padding:"10px 22px",borderRadius:B.radiusXs,background:x.color,color:"#fff",
                    fontSize:13.5,fontWeight:700,textDecoration:"none",transition:"opacity .15s",
                    display:"inline-flex",alignItems:"center",gap:6}}
                  onMouseEnter={e=>e.currentTarget.style.opacity="0.85"}
                  onMouseLeave={e=>e.currentTarget.style.opacity="1"}>
                  {x.label} <ExternalLink size={13}/>
                </a>
              ))}
            </div>
          </div>
